...
local driveVelConstraint  = nil   -- LinearVelocity constraint
local driveVelAttachment  = nil   -- Attachment on HRP
...
-- Lưu ý: isBodyMover dùng để destroy body movers của GAME.
-- Constraint của Hyko (driveVelConstraint) không được destroy vì có Name riêng.
...
local function createVelConstraint(hrp)
    ...
    -- X, Z: max force rất lớn -> khóa cứng vận tốc ngang
    -- Y: 0 -> không can thiệp trọng lực
...
local function driveHeartbeat()
    ...
    -- Anti-ragdoll: force healthy + destroy body + ensure constraint
    if antiRagdollOn then
        ...
        -- Force client network ownership every frame
    ...
    -- Kill body movers & secondary parts physics
    ...
    if not returningHome then
        ...
        if useConstraint then
            -- Khóa cứng vận tốc ngang, game không thể đẩy lùi
...
local function drivePostSim()
    ...
    -- Với LinearVelocity MaxAxesForce XZ=1e6, vận tốc ngang đã bị khóa.
    -- Post-check này là lớp bảo hiểm nếu game tự set AssemblyLinearVelocity.
...
local function startDrive()
    ...
    -- Client network ownership -> server không đẩy được mình
