--========================================================================--
-- Hyko by Huy - WindUI Edition (White Theme + User Info + Beautiful Icons)
-- UI: WindUI by Footagesus (chính chủ)
-- Main: Speed + Anti-Ragdoll + Fast Loot + Hitbox + Return Home
-- Visual: ESP Players + FPS Boost (Ultra++) + FPS Display
...
local function driveHeartbeat()
    ...
    if antiRagdollOn then applyHideBody() end
...
local function refreshDrive()
    ...
    elseif not anyDriveRequested() and driveActive then
        stopDrive()
    elseif driveActive and not antiRagdollOn then
        restoreHideBody()
    end
end
...
local function enableLoot()
    ...
    for _, d in ipairs(workspace:GetDescendants()) do applyFastPrompt(d) end
...
local function reduceAllMeshParts()
    for _, d in ipairs(workspace:GetDescendants()) do
        if d:IsA("MeshPart") then
            pcall(function()
                if Enum.RenderFidelity and Enum.RenderFidelity.Performance then
...
local function createReturnHomeButton()
    ...
    wrap.MouseEnter:Connect(function()
        ...
        TweenService:Create(subtitle, TweenInfo.new(0.15), {
            TextColor3 = Color3.fromRGB(230, 235, 250)
        }):Play()
        TweenService:Create(chev, TweenInfo.new(0.15), {
            TextColor3 = Color3.fromRGB(255, 255, 255)
        }):Play()
        TweenService:Create(ws, TweenInfo.new(0.15), {
            Color = Color3.fromRGB(70, 130, 230)
        }):Play()
        TweenService:Create(iconWrap, TweenInfo.new(0.15), {
            BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        }):Play()
        TweenService:Create(iconText, TweenInfo.new(0.15), {
    ...
    wrap.MouseLeave:Connect(function()
        ...
        TweenService:Create(subtitle, TweenInfo.new(0.15), {
            TextColor3 = Color3.fromRGB(150, 154, 160)
        }):Play()
        TweenService:Create(chev, TweenInfo.new(0.15), {
            TextColor3 = Color3.fromRGB(180, 184, 190)
        }):Play()
        TweenService:Create(ws, TweenInfo.new(0.15), {
    ...
    wrap.MouseButton1Down:Connect(function()
        TweenService:Create(wrap, TweenInfo.new(0.08), {
...
local function setHome()
    ...
    local hrp = c and c:FindFirstChild("HumanoidRootPart")
    if not hrp then
        notify("Hyko by Huy", "Character not found", "error")
        return
    end
...
local function beginReturnHome()
    if not homePos then
        notify("Hyko by Huy", "No home position set", "error")
        return
    end
    ...
    returningHome = true
    startHomeLoop()
    notify("Hyko by Huy", "Returning to home...", "navigation")
end

local function cancelReturnHome()
    if not returningHome then
        notify("Hyko by Huy", "Not currently returning home", "info")
        return
    end
    returningHome = false
    stopHomeLoop()
...
local function formatJobId(id)
    ...
    if #str > 12 then return str:sub(1, 10) .. "..." end
    return str
end
...
local function hopOnce()
    ...
    if not serverId then
        notify("Hyko Server Hop", "Failed to fetch server list", "error")
        return false
    end
    local ok, err = teleportToServer(serverId)
    if not ok then
        notify("Hyko Server Hop", "Error: " .. tostring(err):sub(1, 60), "error")
        return false
    end
    return true
end
...
local Window = WindUI:CreateWindow({
    ...
    KeySystem = {
        Note = "Nhập key để tiếp tục (key: OP)",
        SaveKey = false,
        Key = { "OP" },
    },
})
...
MainTab:Slider({
    ...
    Desc = "Tốc độ hiển thị trên Humanoid giả",
    Icon = "footprints",
    Step = 1,
...
MainTab:Toggle({
    Title = "Enable Speed",
    Desc = "Bật hệ thống di chuyển tốc độ cao",
    Icon = "zap",
    Type = "Checkbox",
    Value = false,
...
MainTab:Section({ Title = "Anti-Ragdoll", Box = true })

MainTab:Slider({
    Title = "Anti-Ragdoll Speed",
    Desc = "Tốc độ khi chống ragdoll",
    Icon = "shield",
    Step = 1,
    Value = { Min = 10, Max = 800, Default = 60 },
...
MainTab:Toggle({
    Title = "Enable Anti-Ragdoll (Hide Body)",
    Desc = "Ẩn toàn bộ bộ phận cơ thể, chống ragdoll",
    Icon = "shield-check",
    Type = "Checkbox",
    Value = false,
    Callback = function(state)
        antiRagdollOn = state
        refreshDrive()
        notify("Hyko by Huy", state and "Anti-Ragdoll enabled" or "Anti-Ragdoll disabled",
...
MainTab:Toggle({
    ...
    Desc = "Đặt HoldDuration = 0 và tự động kích hoạt prompt gần nhất",
    Icon = "package",
    Type = "Checkbox",
    Value = false,
    Callback = function(state)
...
MainTab:Slider({
    Title = "Hitbox Size",
    Desc = "Kích thước hitbox của người chơi khác",
    Icon = "target",
    Step = 1,
...
MainTab:Toggle({
    ...
    Desc = "Mở rộng hitbox của tất cả người chơi khác",
    Icon = "crosshair",
    Type = "Checkbox",
    Value = false,
    Callback = function(state)
...
MainTab:Toggle({
    ...
    Desc = "Hiển thị nút nổi trên màn hình",
    Icon = "home",
    Type = "Checkbox",
    Value = false,
    Callback = function(state)
        if state then
            ...
            task.spawn(function()
                while ReturnBtn do
                    if returnBtnConn then break end
                    returnBtnConn = ReturnBtn.MouseButton1Click:Connect(function()
                        beginReturnHome()
                    end)
                    break
                end
...
MainTab:Button({
    ...
    Desc = "Lưu vị trí hiện tại làm nhà",
    Icon = "map-pin",
...
MainTab:Button({
    Title = "Return Home",
    Desc = "Quay về vị trí nhà đã lưu",
    Icon = "home",
...
MainTab:Button({
    ...
    Desc = "Dừng quá trình quay về nhà",
    Icon = "stop-circle",
...
MainTab:Paragraph({
    Title = "Notes",
    Desc = "Speed & Anti-Ragdoll dùng chung hệ thống drive với Humanoid giả local.\n"
        .. "Anti-Ragdoll ẩn toàn bộ bộ phận cơ thể.\n"
        .. "Fast Loot đặt HoldDuration = 0 và tự động kích hoạt prompt gần nhất.\n"
        .. "Hitbox Expand mở rộng hitbox người chơi khác.\n"
...
VisualTab:Toggle({
    ...
    Desc = "Highlight và tên phía trên người chơi khác",
    Icon = "radar",
    Type = "Checkbox",
    Value = false,
    Callback = function(state)
...
VisualTab:Toggle({
    ...
    Desc = "Giảm đồ hoạ mạnh + làm mờ nền",
    Icon = "rocket",
    Type = "Checkbox",
    Value = false,
    Callback = function(state)
        if state then
            ...
            if not ok then
                notify("Hyko by Huy", "FPS Boost error: " .. tostring(err):sub(1, 60), "error")
            else
...
VisualTab:Button({
    ...
    Desc = "Xoá thủ công particles, lights, shadows",
    Icon = "sparkles",
    Callback = function()
        pcall(scanAndRemoveEffects)
        pcall(reduceAllMeshParts)
...
VisualTab:Toggle({
    ...
    Desc = "Hiển thị overlay FPS/ping có glow trắng",
    Icon = "activity",
    Type = "Checkbox",
    Value = false,
    Callback = function(state)
...
VisualTab:Paragraph({
    ...
    Desc = "Đặt QualityLevel tối thiểu, FramerateCap 240.\n"
        .. "Xoá tất cả lighting effects, shadows, post-processing, skybox children.\n"
        .. "Sương mù trắng (FogStart 55 / FogEnd 210) ẩn vật thể xa.\n"
        .. "Thêm BlurEffect size 24 + ColorCorrection để làm mờ nền.\n"
        .. "Xoá Particle / Trail / Beam / Smoke / Fire / Sparkles / Explosion.\n"
        .. "Tắt tất cả Point / Spot / Surface lights.\n"
        .. "Giảm chi tiết nước và MeshPart render fidelity.\n"
        .. "Chuyển toàn bộ Material sang SmoothPlastic.",
    Icon = "info",
})
...
HopTab:Input({
    ...
    Desc = "Số người chơi tối thiểu để kích hoạt hop",
...
HopTab:Button({
    Title = "Hop Once",
    Desc = "Teleport đến server public ngẫu nhiên",
    Icon = "refresh-cw",
...
HopTab:Section({ Title = "Server Browser", Box = true })

HopTab:Button({
    Title = "Open Server Browser",
    Desc = "Duyệt, sắp xếp và tham gia server public",
    Icon = "globe",
    Callback = function()
        local BrowserWindow = WindUI:CreateWindow({
        ...
        local BrowserTab = BrowserWindow:Tab({ Title = "Servers", Icon = "server" })
        BrowserTab:Section({ Title = "Options", Box = true })

        BrowserTab:Dropdown({
            Title = "Sort Order",
            Desc = "Sắp xếp theo số người chơi",
            Icon = "arrow-up-down",
            Values = { "Players (Low to High)", "Players (High to Low)" },
            Value = "Players (Low to High)",
            Callback = function(opt)
                if opt == "Players (High to Low)" then
        ...
        local serverDropdown = BrowserTab:Dropdown({
            Title = "Servers",
            Desc = "Chọn server để tham gia",
            Icon = "list",
            Values = { "Loading..." },
            Value = "Loading...",
        ...
        local browserInfoPara = BrowserTab:Paragraph({
            Title = "Selected Server",
            Desc = "No server selected",
            Icon = "info",
        })

        BrowserTab:Button({
            Title = "Refresh Server List",
            Desc = "Tải danh sách server mới",
            Icon = "refresh-cw",
            Callback = function()
                task.spawn(function()
                    pcall(function()
                    ...
                    if not data or #data == 0 then
                        ...
                        pcall(function()
                            browserInfoPara:SetDesc("No servers found")
                        end)
                        return
                    end
                    ...
                    if #opts == 0 then opts = { "No servers found" } end
                    pcall(function()
        ...
        BrowserTab:Button({
            ...
            Desc = "Tham gia server đã chọn",
            Icon = "log-in",
            Callback = function()
                local selected = serverDropdown.Value
                if not selected then
                    notify("Server Browser", "No server selected", "error")
                    return
                end
                local map = serverCache._map
                if not map or not map[selected] then
                    notify("Server Browser", "Invalid selection", "error")
                    return
                end
                local jobId = map[selected]
                pcall(function()
                    browserInfoPara:SetDesc("Joining " .. jobId)
                end)
                local ok, err = teleportToServer(jobId)
                if not ok then
                    notify("Server Browser", "Error: " .. tostring(err):sub(1, 60), "error")
                end
            end,
        })

        BrowserTab:Section({ Title = "Info", Box = true })
        BrowserTab:Paragraph({
            Title = "How to use",
            Desc = "1. Click Refresh Server List.\n"
...
HopTab:Paragraph({
    Title = "Notes",
    Desc = "Hop Once: teleport đến server public ngẫu nhiên nếu số người chơi trên ngưỡng.\n"
        .. "Auto Hop: lặp lại hop mỗi vài giây.\n"
        .. "Server Browser: liệt kê, sắp xếp, tìm kiếm và tham gia server public.",
    Icon = "info",
})
...
print("[Hyko] ✅ All loaded successfully - WindUI Edition (White Theme)")