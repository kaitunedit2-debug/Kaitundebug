...
--========================================================================--
-- ANTI-RAGDOLL CORE: Destroy mọi BasePart trừ HumanoidRootPart
--========================================================================--
...
-- Không thể khôi phục part đã Destroy. Chỉ reset state khi tắt.
local function restoreAntiRagdollDestroy()
    -- No-op. Body chỉ trở lại khi nhân vật respawn.
end
...
local function driveHeartbeat()
    ...
    if antiRagdollOn then applyAntiRagdollDestroy() end
...
local function startDrive()
    ...
    for _, p in ipairs(c:GetDescendants()) do
        if p:IsA("BasePart") and p ~= hrp then
            if antiRagdollOn then
                -- Anti-ragdoll bật: destroy luôn part khác
                pcall(function() p:Destroy() end)
            else
    ...
    c.DescendantAdded:Connect(function(d)
        ...
        if d:IsA("BasePart") and d.Name ~= "HumanoidRootPart" then
            if antiRagdollOn then
                -- Part mới thêm khi anti-ragdoll bật -> destroy luôn
                task.defer(function()
...
--========================================================================--
-- [10] RETURN HOME BUTTON (Stable mobile button)
--========================================================================--
...
MainTab:Toggle({
    Title = "Enable Anti-Ragdoll (Destroy Body)",
    Desc = "Destroys all body parts except HumanoidRootPart to prevent ragdoll",
    Icon = "shield-check",
    Type = "Checkbox",
    Value = false,
    Callback = function(state)
        ...
        refreshDrive()
        notify("Hyko by Huy",
            state and "Anti-Ragdoll enabled (body destroyed)"
                  or "Anti-Ragdoll disabled (respawn to restore body)",
...
MainTab:Paragraph({
    ...
    Desc = "Speed & Anti-Ragdoll share a single drive system using a local fake Humanoid.\n"
        .. "Anti-Ragdoll DESTROYS all body parts except HumanoidRootPart. Respawn to restore body.\n"
...
print("[Hyko] All loaded successfully - WindUI Edition (Optimized)")      end)
        else
            if autoHopThread then
                pcall(function() task.cancel(autoHopThread) end)
                autoHopThread = nil
